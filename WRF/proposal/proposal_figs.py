...
def SWE_fig(data):
    ...
    for idx, years in enumerate(time_ranges):
        ...
        # set map extend
        extent = [float(SWE_mean['XLONG'][0][0]), float(SWE_mean['XLONG'][0][-1]), float(SWE_mean['XLAT'][2][0]), float(SWE_mean['XLAT'][-1][0])]
...
def snow_fraction(SR_data, HGT_data):
    ...
    norm = mcolors.TwoSlopeNorm(vmin = 0, vmax = 1, vcenter = 0.9) 
